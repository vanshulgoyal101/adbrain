> Historical design and evidence preserved September 26, 2026. Old onboarding claims, proposed APIs, commands and sequencing are not current instructions; use [the current payment guide](PAYMENTS-PLAN.md). Transaction receipts below retain their original scope.

# Customer Payments and Managed Advertising

Status: local test checkout implemented; Razorpay live account/settlement status
verified; AdBrain production checkout still requires implementation. Updated:
2026-09-26. Owner requests live-payment rollout. No live charge, bank transfer,
Meta funding, credential change or production migration was performed by this check.

## Verified Razorpay Account Status

September 26, authenticated read-only dashboard inspection after owner sign-in:

- Dashboard explicitly shows "You are in live mode".
- Activation details show "Account Activated On: Sep 26 2026, 01:30 pm" and
   "Account Access: Complete". The displayed time's timezone was not established.
- Bank account details show "Active bank account" and "Settlements: ACTIVE".
- Business website details list `https://adbrain.vanshul.com` as "Approved".
- No API keys, bank identifiers, identity documents or account values are recorded
   here. No settings were changed and no payment, refund or settlement was triggered.

Merchant activation, an active settlement account and website approval are no
longer unknown blockers. This does not prove an actual bank settlement, production
webhook delivery, live AdBrain checkout, refund execution, merchant approval for
every proposed funds model, or automatic Meta funding. Reuse this status evidence;
do not repeat merchant onboarding merely because older notes below say pending.
Next payment work is a production order/entitlement and verification flow using
the existing Razorpay SDK, not removal of the test-only guards or a new SDK rewrite.
...
eligibility remains unverified. Razorpay account activation and active settlement
status are now verified above; proposed managed-funds use and the live application
flow are separate requirements. No address, tax ID or contact details from screenshots are
...
| Gateway | Razorpay Standard Checkout; live account access and active settlement account verified September 26 | Production integration and applicable business-model approval; actual settlement/refund evidence remains separate |
